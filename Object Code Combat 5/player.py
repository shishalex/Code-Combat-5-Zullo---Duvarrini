...
class Player:
    ...
    def __init__(self, name: str = choice(names) + " " + choice(surnames),  max_health: int = randint(90, 120), strength: int = randint(1, 20), dexterity: int = randint(1, 20)):
        ...
        if max_health <= 0:
            raise ValueError("Gli HP massimi devono essere maggiore di 0.")
        if strength <= 0:
            raise ValueError("La forza deve essere maggiore di 0.")
        if dexterity <= 0:
            raise ValueError("La destrezza deve essere maggiore di 0.")
    ...
    def weapon(self, weapon: Weapon) -> None:
        if not hasattr(weapon, "get_damage") or not callable(weapon.get_damage):
            raise TypeError("Oggetto non equipaggiabile, non è un’arma valida.")

        if not hasattr(weapon, "w_type") or not isinstance(weapon.w_type, str):
            raise TypeError("Oggetto non equipaggiabile, non è un’arma valida.")
